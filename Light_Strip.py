...
def serial_out(C):
    '''
        Pass an ascii encodeded variable throuh the usb serial connection
    '''
    usb.write(C.encode())
...
# Add buttons for each color and an off button
off_switch = Button(window, width=buttonWidth, text="Off",
                    foreground="White", background="Black",
                    command=lambda: serial_out('0')).grid(row=1)
White = Button(window, width=buttonWidth, text="White",
               background="White",
...
Violet = Button(window, width=buttonWidth, text="Violet",
                background="Purple",
                command=lambda: serial_out('8')).grid(row=9)
